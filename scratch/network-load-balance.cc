...
// Use the tuple_hash from settings.h for unordered_map with tuple keys
// #include "ns3/point-to-point-module.h"
// #include "src/point-to-point/model/settings.h" // 使用完整相对路径

// using ns3::Settings::tuple_hash;
...
std::map<uint32_t, std::map<uint32_t, std::vector<uint32_t>>> ConvertAndStore(const std::map<Ptr<Node>, std::map<Ptr<Node>, std::vector<Ptr<Node>>>>& nextHop){
    ...
    for (std::map<Ptr<Node>, std::map<Ptr<Node>, std::vector<Ptr<Node>>>>::const_iterator outerIter = nextHop.begin(); outerIter != nextHop.end(); ++outerIter) {
        Ptr<Node> outerKey = outerIter->first;
        const std::map<Ptr<Node>, std::vector<Ptr<Node>>>& innerMap = outerIter->second;
        uint32_t outerId = outerKey->GetId();
        for (std::map<Ptr<Node>, std::vector<Ptr<Node>>>::const_iterator innerIter = innerMap.begin(); innerIter != innerMap.end(); ++innerIter) {
            Ptr<Node> innerKey = innerIter->first;
            const std::vector<Ptr<Node>>& value = innerIter->second;
            uint32_t innerId = innerKey->GetId();
            
            std::vector<uint32_t> idVector;
            for (std::vector<Ptr<Node>>::const_iterator vecIter = value.begin(); vecIter != value.end(); ++vecIter) {
                Ptr<Node> nodePtr = *vecIter;
...
std::map<uint32_t, std::map<uint32_t, uint32_t>> CreateNodeInterfaceMap(const std::map<Ptr<Node>, std::map<Ptr<Node>, Interface>>& nbr2if){
    ...
    for (std::map<Ptr<Node>, std::map<Ptr<Node>, Interface>>::const_iterator outerIter = nbr2if.begin(); outerIter != nbr2if.end(); ++outerIter) {
        Ptr<Node> node = outerIter->first;
        const std::map<Ptr<Node>, Interface>& ifMap = outerIter->second;
        int nodeId = node->GetId();
        
        for (std::map<Ptr<Node>, Interface>::const_iterator innerIter = ifMap.begin(); innerIter != ifMap.end(); ++innerIter) {
            Ptr<Node> nbrNode = innerIter->first;
            const Interface& iface = innerIter->second;
...
std::map<uint32_t, std::map<uint32_t, uint32_t>> CreateNbr2Interface(const std::map<Ptr<Node>, std::map<Ptr<Node>, Interface>>& nbr2if){
    std::map<uint32_t, std::map<uint32_t, uint32_t>> Id_nbr2if;
    for (std::map<Ptr<Node>, std::map<Ptr<Node>, Interface>>::const_iterator outerIter = nbr2if.begin(); outerIter != nbr2if.end(); ++outerIter) {
        Ptr<Node> node = outerIter->first;
        const std::map<Ptr<Node>, Interface>& ifMap = outerIter->second;
        int nodeId = node->GetId();
...
void ReadFlowInput() {
    if (flow_input.idx < flow_num) {
        flowf >> flow_input.src >> flow_input.dst >> flow_input.pg >> flow_input.maxPacketCount >>
            flow_input.start_time >> flow_input.reorderable;
...
void ScheduleFlowInputs(FILE *infile) {
    ...
    while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()) {
        ...
        // Store reorderable
        Settings::reorderable[std::make_tuple(serverAddress[src].Get(), serverAddress[dst].Get(), static_cast<uint16_t>(sport), static_cast<uint16_t>(dport))] = flow_input.reorderable;
        ...
        if (lb_mode == 9 || lb_mode == 12 || lb_mode == 3 || lb_mode == 6){
            ...
            if (it != SrcId2CurSrcToR.end()){
                ...
                if (SrcDstToR_log){
                    ...
                    std::map<Ptr<Node>, Interface> show_innerMap = nbr2if[n.Get(src)];
                    for (std::map<Ptr<Node>, Interface>::iterator show_innerIter = show_innerMap.begin(); show_innerIter != show_innerMap.end(); ++show_innerIter) {
                        Ptr<Node> innerKey = show_innerIter->first;
                        Interface value = show_innerIter->second;
