...
class RdmaRxQueuePair : public Object {  // Rx side queue pair
   public:
    ...
    uint32_t m_window_start;  // 窗口起始序号
    std::map<uint32_t, Ptr<Packet>> m_reorder_buffer;  // 重排缓冲区，使用 map 以序列号为键
    bool m_reorderable;  // 是否支持重排 (从 Settings::reorderable 获取)
    EventId m_window_timer;  // 窗口超时计时器
    Time m_window_timeout;  // 超时时间 (可设置为一个常量，如 1ms，根据需求调整)
